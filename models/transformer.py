...
class TransformerDecoder(nn.Module):
    def __init__(self, input_dim, dec_dim, nhead, num_layers, dim_feedforward, dropout=0.1):
        super(TransformerDecoder, self).__init__()

        # Embedding layer for the original input
        self.embedding = nn.Linear(input_dim, dec_dim)

        # Transformer decoder layers
        decoder_layer = nn.TransformerDecoderLayer(dec_dim, nhead, dim_feedforward, dropout)
        self.transformer_decoder = nn.TransformerDecoder(decoder_layer, num_layers)

        # Linear projection layer to match the dimensions of the encoder output
        self.projection = nn.Linear(dec_dim, input_dim)

    def forward(self, x, enc_output, enc_features):
        x += enc_output
        # Embed the original input
        embedded_x = self.embedding(x)
        enc_features = enc_features.permute(1, 0, 2)  # Reshape to [56, 1024, 256]
        # Apply the transformer decoder
        decoded_output = self.transformer_decoder(embedded_x, enc_features)

        # Project the decoded output back to the original dimension
        projected_output = self.projection(decoded_output)
